...
def create_combined_report(csv_file_path, json_file_path, output_file_path=None):
    ...
    last_123_sum = last_123_days.groupby(['department', 'Article'])['target'].sum().reset_index()
    last_123_sum['last_123_days_sum'] = last_123_sum['target'].apply(process_number)
    last_123_sum = last_123_sum[['department', 'Article', 'last_123_days_sum']]
    ...
    for segment, forecasts in forecast_data.items():
        ...
        department = segment.split('|')[0] if '|' in segment else 'Unknown'
        ...
        forecast_rows.append({
            ...
            'Article': article,
            'forecast_123_days_mean': process_number(total_mean),
            'forecast_123_days_lower': process_number(total_lower),
            'forecast_123_days_upper': process_number(total_upper)
        })
    ...
    historical_monthly = df.groupby(['department', 'Article', 'month'])['target'].sum().reset_index()
    ...
    forecast_monthly = forecast_df.groupby(['department', 'Article', 'month']).agg({
        'target_mean': 'sum',
        'target_lower': 'sum',
        'target_upper': 'sum'
    }).reset_index()
    ...
    combined_df = combined_df.merge(
        forecast_mean_pivot.rename(columns={'department': 'Департамент', 'Article': 'Артикул'}),
        on=['Департамент', 'Артикул'],
        how='outer'
    )

    combined_df = combined_df.merge(
        forecast_lower_pivot.rename(columns={'department': 'Департамент', 'Article': 'Артикул'}),
        on=['Департамент', 'Артикул'],
        how='outer'
    )

    combined_df = combined_df.merge(
        forecast_upper_pivot.rename(columns={'department': 'Департамент', 'Article': 'Артикул'}),
        on=['Департамент', 'Артикул'],
        how='outer'
    )
    ...
    combined_df = combined_df.merge(
        last_123_sum.rename(columns={'department': 'Департамент', 'Article': 'Артикул'}),
        on=['Департамент', 'Артикул'],
        how='left'
    )
    ...
    combined_df = combined_df.merge(
        forecast_sums.rename(columns={'department': 'Департамент', 'Article': 'Артикул'}),
        on=['Департамент', 'Артикул'],
        how='left'
    )
    ...
    numeric_columns = [col for col in combined_df.columns if col not in ['Департамент', 'Артикул']]
    for col in numeric_columns:
        combined_df[col] = combined_df[col].apply(process_number)

    # Сортировка колонок - итоговые колонки в конец
    all_columns = combined_df.columns.tolist()
    dept_article_cols = ['Департамент', 'Артикул']
    summary_cols = ['last_123_days_sum', 'forecast_123_days_mean', 'forecast_123_days_lower', 'forecast_123_days_upper']
    ...
    # Статистика
    print(f"\nСтатистика отчета:")
    print(f"Уникальных департаментов: {combined_df['Департамент'].nunique()}")
    ...
    print(
        f"Итоговые колонки: last_123_days_sum, forecast_123_days_mean, forecast_123_days_lower, forecast_123_days_upper")
...
def main():
    ...
    csv_file = "20230701_20250630.csv"
    json_file = "forecast.json"
    output_file = "combined_sales_forecast_report.csv"
