...
lags_small = LagTransform(in_column="target", lags=[14,21,28,30,31], out_column="target_lag_small")
...
stl = STLTransform(in_column="target", model="arima", period=91, robust=True, model_kwargs={"order": (2,1,1)})
trend = LinearTrendTransform(in_column="target")
imputer = TimeSeriesImputerTransform(in_column="target", strategy="seasonal")
mean_tr_1 = MeanTransform(in_column="target", alpha=1, fillna=0, seasonality=1, min_periods=1, out_column="mean_1", window=1)
mean_tr_3 = MeanTransform(in_column="target", alpha=1, fillna=0, seasonality=3, min_periods=3, out_column="mean_3", window=3)
mean_tr_7 = MeanTransform(in_column="target", alpha=1, fillna=0, seasonality=7, min_periods=7, out_column="mean_7", window=7)
...
std_1 = StdTransform(in_column="target", seasonality=1, window=1, fillna=0, min_periods=1, out_column="std_1", ddof=1)
std_3 = StdTransform(in_column="target", seasonality=3, window=3, fillna=0, min_periods=3, out_column="std_3", ddof=1)
std_7 = StdTransform(in_column="target", seasonality=7, window=7, fillna=0, min_periods=7, out_column="std_7", ddof=1)
...
#    STLTransform(in_column="target", model="arima", period=7, robust=False, model_kwargs={"order": (2,2,2)}),
...
def objective(trial):
    best_params = {
        'iterations': trial.suggest_int('iterations', 873, 873),
        'learning_rate': trial.suggest_float('learning_rate', 0.6112990073841404, 0.6112990073841404, log=True),
        'depth': trial.suggest_int('depth', 10, 10),
        'l2_leaf_reg': trial.suggest_float('l2_leaf_reg', 9.29277478806184, 9.29277478806184, log=True),
        'random_strength': trial.suggest_float('random_strength', 0.07240417120087717, 0.07240417120087717),
        'bagging_temperature': trial.suggest_float('bagging_temperature', 1.5372496609872792, 1.5372496609872792),
        'border_count': trial.suggest_int('border_count', 41, 41),
        'one_hot_max_size': trial.suggest_int('one_hot_max_size', 21, 21),
